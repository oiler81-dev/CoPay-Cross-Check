// Copay Cross-Check
// Tailored to:
/// EligibilityFinancialSummaryReport: Patient Name, DOB, Spec.Copay (In-Net), Location
/// Patient Balance Report: Patient Name (First Last), Patient Birth Date, Patient Payments, Appointment User
...
let LAST_RESULTS = [];     // patient-level
let LAST_USER_ROWS = [];   // per-user
let LAST_META = {};        // export support
...
function toISODateLoose(v) {
  // goal: make DOB comparisons stable across excel date + text date
  ...
  if (m) {
    ...
    let yy = m[3];
    if (yy.length === 2) yy = "20" + yy; // best effort
    return `${yy}-${mm}-${dd}`;
  }
  return "";
}
...
function matchConfidence(eligName, balName) {
  // Weighted: last name matters more.
  ...
  // Bonus if one name contains the other (handles middle names, suffix)
...
function bestNameMatchRow(targetName, rows, nameField) {
  // pick highest confidence, return { row, confidence, notes }
  let best = null;
  let bestScore = -1;
  let notes = "";
...
async function run() {
  ...
  try {
    ...
    // Normalize + validate required columns exist (best effort)
    const elig = normalizeEligibility(eligRows);
    const bal = normalizeBalance(balRows);

    setStatus(`Loaded: Eligibility ${elig.length.toLocaleString()} rows, Balance ${bal.length.toLocaleString()} rows. Matching...`);

    const results = crossCheck(elig, bal);
    LAST_RESULTS = results;

    // Build location filter options from results
    ...
    setStatus(`Done. Matched ${results.length.toLocaleString()} copay-expected patients.`);
  } catch (err) {
    console.error(err);
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Use array-of-arrays so we can detect header row
...
function detectHeaderRowIndex(aoa) {
  ...
  // we score rows that contain key strings like "Patient Name", "DOB", "Spec.Copay" or "Patient Name (First Last)" etc.
  const keys = [
    "patient name",
    "dob",
    "spec.copay",
  ...
  return bestIdx; // works for your Eligibility file where headers start at row index 2
}
...
function normalizeEligibility(rows) {
  // Expect: Patient Name, DOB, Spec.Copay (In-Net), Location
  const out = [];
...
function normalizeBalance(rows) {
  // Expect: Patient Name (First Last), Patient Birth Date, Patient Payments, Appointment User
  const out = [];
...
function crossCheck(elig, bal) {
  // Index balance rows by DOB for faster search
  const byDob = new Map();
  for (const b of bal) {
  ...
  for (const e of elig) {
    ...
    if (candidates.length > 0) {
      ...
      if (matchedRow) {
        collected = matchedRow.collected || 0;
        user = matchedRow.user || "Unknown";
      }
    }

    // Status rules:
    // - Collected: collected >= expected
    // - Partial: 0 < collected < expected
    // - Not Collected: collected == 0 (or no match)
    ...
    results.push({
      ...
      confidence,
      matchNotes: candidates.length ? notes : "No DOB match",
...
function buildLocationOptions(results) {
  ...
  // reset options
  els.locationFilter.innerHTML = `<option value="All">All locations</option>` +
...
function renderAll() {
  ...
  // Wire filter events once (idempotent)
...
function applyFilters() {
  ...
  renderKPIs(filtered, LAST_RESULTS); // show KPIs for filtered but keep global avg fallback
...
function renderKPIs(results, global = null) {
  ...
  // If filtered is empty but global exists, keep confidence meaningful
...
function exportResults() {
  ...
  // Sheet 3: Summary Meta
  const summary = buildSummaryRows(LAST_RESULTS);
  const meta = [
    { Key: "GeneratedAt", Value: LAST_META.generatedAt || "" },
    { Key: "EligibilityRowsUsed", Value: LAST_META.eligRows || 0 },
    { Key: "BalanceRowsUsed", Value: LAST_META.balRows || 0 },
    ...summary
  ];
...
function buildSummaryRows(results) {
  ...
  return [
    { Key: "PatientsWithCopayExpected", Value: patients },
    { Key: "CollectedCount", Value: collectedCount },
    { Key: "PartialCount", Value: partialCount },
    { Key: "NotCollectedCount", Value: notCount },
    { Key: "ExpectedTotal", Value: expectedTotal },
    { Key: "CollectedTotal", Value: collectedTotal },
    { Key: "NotCollectedTotal", Value: notCollectedTotal },
    { Key: "AvgMatchConfidence", Value: avgConfidence },
  ];
}
